...
spec =
  describe "parser" $ do
    ...
    it "test definition" $
      parseKoak "def add(num1:int num2:int):int num1+num2;"
        `shouldBe` Right
          ( KStmt
              [ KDefs
                  ( KPrototype
    ...
    it "test if then else" $
      parseKoak "if 1<2 then 1 else 2;"
        `shouldBe` Right
          ( KStmt
              [ KExpressions $
                  KIfElse
    ...
    it "test for" $
      parseKoak "for i=0,i<10,i=i+1 in res=res+i*2;"
        `shouldBe` Right
          ( KStmt
              [ KExpressions $
                  KFor
